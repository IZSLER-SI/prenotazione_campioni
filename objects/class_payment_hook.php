<?php  
class prenotazione_campioni_paymentHook{
	public $conn;
	public function payment_purchase_status() {
		$payment_extensions_res=$this->get_p_pay_option('ct_payment_extensions');
		$final_result = unserialize($payment_extensions_res);
		$check_arr = array();
		if(isset($final_result) && $final_result !== false){
			foreach($final_result as $key=>$value) {
				$ext_pstatus = "ct_".base64_encode($key."_pstatus");
	...
	public function get_p_pay_option($option_name){
		$query = "select `option_value` from `ct_settings` where `option_name`='" . $option_name . "'";
