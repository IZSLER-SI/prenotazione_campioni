<?php  
class prenotazione_campioni_dayweek_avail{
   ...
   /* Monthly schedule*/
        ...
        public function getinterval()
        {
            ...
            if(isset($_SESSION['ct_laboratorioid'])){
                ...
                $query="select slot from izler_laboratori
                join ct_admin_info on izler_laboratori.id = ct_admin_info.lab
                where ct_admin_info.id = ".$lab_user;
            }else{
...
/*             echo $query; */
    ...
    public function get_timeavailability_check()
    {
        ...
        return $result;
            ...
            /* Get the time interval from the settings table */
    public function getNumeroCampioni()
    {
        
        if(isset($_SESSION['ct_laboratorioid'])){
            $lab_user = $_SESSION['ct_laboratorioid'];
            $query="select campioni_ufficiale from izler_laboratori
            join ct_admin_info on izler_laboratori.id = ct_admin_info.lab
            where ct_admin_info.id = ".$lab_user;
        }else{
            $query="select 1 as campioni_ufficiale";
        }
        $result=mysqli_query($this->conn,$query);
        $value=mysqli_fetch_row($result);
        return $value;
