...
$timeavailability = new prenotazione_campioni_dayweek_avail();
$timeavailability->conn = $conn;
$taxamount = ""; /* add item in to cart */

$services_addon = new prenotazione_campioni_services_addon();
$services_addon_rates = new prenotazione_campioni_services_addon_rates();
$services_methods_units = new prenotazione_campioni_services_methods_units();
$services_methods_units_rates = new prenotazione_campioni_services_methods_units_rates();
...
elseif (isset($_POST['check_for_option']))
	...
	if ($settings->get_option('ct_company_address') == "" || $settings->get_option('ct_company_city') == "" || $settings->get_option('ct_company_state') == "" || $settings->get_option('ct_company_name') == "" || $settings->get_option('ct_company_email') == "" || $settings->get_option('ct_company_zip_code') == "" || $settings->get_option('ct_company_country') == "" || mysqli_num_rows($hh) == "" || mysqli_num_rows($t) == "")
		{
		$last = "Please complete configurations before you created prenotazione_campioni website embed code. ";
		}
