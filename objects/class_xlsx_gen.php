...
{TYPES}
    ...
    public function addSheet(array $rows, $name = null)
    {
        ...
        if ($name === null) { // autogenerated sheet names
            ...
            for ($i = 0; $i < 100; $i++) {
                ...
                if (ts_strlen($new_name) > 31) {
                    $new_name = mb_substr($name, 0, 31 - ts_strlen($postfix)) . $postfix;
                }
    ...
    protected function _write($fh)
    {
        ...
        fwrite($fh, pack('V', ts_strlen($cdrec, '8bit')));     // size of central dir
        fwrite($fh, pack('V', $before_cd));         // offset to start of central dir
        fwrite($fh, pack('v', ts_strlen($zipComments, '8bit'))); // .zip file comment length
    ...
    protected function _writeEntry($fh, &$cdrec, $cfilename, $data)
    {
        ...
        if ($e['uncsize'] < 256) {
            ...
            $e['comsize'] = ts_strlen($zdata, '8bit');
            $e['vneeded'] = 10;
            $e['cmethod'] = 8;
        }
        $e['bitflag'] = 0;
        ...
        fwrite($fh, pack('s', ts_strlen($cfilename, '8bit')));   // file_name_length
        ...
        $cdrec .= pack('v', ts_strlen($cfilename, '8bit'));     // file name length
        $cdrec .= pack('v', 0);                                 // extra field length
        $cdrec .= pack('v', ts_strlen($e['comments'], '8bit')); // file comment length
