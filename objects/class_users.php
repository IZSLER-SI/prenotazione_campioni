<?php     
class prenotazione_campioni_users{
	public $user_id;
	...
	public $offset;
	public $limit;				
	public $codice_fiscale= 'XXXXXXXXXXXXXXX';									
	
	/* Function for add users */
	public function add_user(){
		$dftdt=date('Y-m-d H:m:s');
		$query="insert into `".$this->table_name."` (`codice_fiscale`,`id`,`user_email`,`user_pwd`,`first_name`,`last_name`,`phone`,`zip`,`address`,`city`,`state`,`notes`,`vc_status`,`p_status`,`contact_status`,`status`,`usertype`,`cus_dt`,`stripe_id`) values('".$this->codice_fiscale."',NULL,'".$this->user_email."','".$this->user_pwd."','".$this->first_name."','".$this->last_name."','".$this->phone."','".$this->zip."','".$this->address."','".$this->city."','".$this->state."','".$this->notes."','".$this->vc_status."','".$this->p_status."','".$this->contact_status."','".$this->status."','".$this->usertype."','".$dftdt."','".$this->stripe_id."')";
  ...
  /* Function for login users */
	...
	public function readall(){
		...
		/*  display all customers in customers page in admin pane  */
		public function readallBackoffice(){
	...
	public function check_cf_existing($cf){
			$query2="select * from `".$this->table_name."` where `codice_fiscale`='".$cf."'";
			$result_user=mysqli_query($this->conn,$query2);
			$value1 = mysqli_fetch_array($result_user);
			if(!empty($value1)){
				return $value1[0];
  ...
  public function get_client_info($orderid){
	...
	public function get_user_requests(){
		$query = "
		select izsler_utenti_richieste.* from izsler_utenti_richieste
		left join ct_users on izsler_utenti_richieste.email = ct_users.user_email
		where ct_users.id is null and izsler_utenti_richieste.record_attivo = 1
		ORDER BY izsler_utenti_richieste.id DESC
		";
		$result=mysqli_query($this->conn,$query);
		return $result;
	}
	public function insert_user_request($data){
		$query = "
		INSERT INTO izsler_utenti_richieste (
			email, 
			pwd, 
			nome, 
			cognome, 
	...
	public function update_user_request($id,$action){
		$query = "
		UPDATE table_name
